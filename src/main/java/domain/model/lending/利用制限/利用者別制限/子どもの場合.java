...
public class 子どもの場合 implements 利用者別利用制限IF {

    final static 冊 最大貸出可能数 = new 冊(7);
    final static 冊 一次許容貸出数 = new 冊(4);
    final static 遅延日数 最大許容遅延日数 = new 遅延日数(7L);
    ...
    public 冊 現在借りられる冊数(貸出本一覧 現在借りている本一覧, 日付 起算日){
        ...
        if (最大遅延日数.graterThanEq(最大許容遅延日数)) return new 冊(0);

        if (最大遅延日数.graterThanEq(new 遅延日数(4L)) && 最大遅延日数.lessThanEq(new 遅延日数(6L))){
            return 一次許容貸出数.引く(現在貸し出している本の数);
        }
